...
-- put data into stage
put
file://C:\Users\User\Downloads\CLASS\customer_detail.csv
...
CREATE OR REPLACE STAGE S3_STAGE_TESLA_STOCKS
URL = "s3://muzammilsnowflakeloadingdemo/TSLA.csv"
CREDENTIALS = (AWS_KEY_ID='-------------', AWS_SECRET_KEY='-----------------');

-- list stage
LIST @S3_STAGE_TESLA_STOCKS;

-- copy data from stage to table
COPY INTO TESLA_STOCKS
	FROM @S3_STAGE_TESLA_STOCKS
...
--copy that ARN: arn:aws:iam::588738579160:role/S3_INTEGRATION_ROLE
...
CREATE OR REPLACE STORAGE INTEGRATION S3_INTEGRATION
  TYPE = EXTERNAL_STAGE
  STORAGE_PROVIDER = 'S3'
  STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::588738579160:role/S3_INTEGRATION_ROLE'
  ENABLED = TRUE
  STORAGE_ALLOWED_LOCATIONS = ('s3://muzammilsnowflakeloadingdemo/input/');
...
CREATE OR REPLACE STAGE S3_INTEGRATEION_BULK_COPY_TESLA_STOCKS
  ...
  URL = 's3://muzammilsnowflakeloadingdemo/input/TSLA.csv'
  FILE_FORMAT = (TYPE = 'CSV', FIELD_DELIMITER=',', SKIP_HEADER=1);
...
-- updating values
UPDATE TESLA_STOCKS SET OPEN_VALUE=200 WHERE DATE = '2021-06-25';

-- getting data beofre last upodate query
SELECT * FROM TESLA_STOCKS BEFORE (statement => '01b75662-0000-bcf9-0007-fd460001a12e') ORDER BY DATE DESC;
...
BV06661.AWS_AP_SOUTH_1.snowflakecomputing.com
